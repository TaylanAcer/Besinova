...
// Vücut ölçüleri, aktivite seviyesi, amaç seçimi ve sonuç kartları içerir.
...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  void _calculateResults() {
    ...
    if (height != null &&
      ...
      // Hesaplanan verileri UserProvider'a kaydet
      _saveCalculatedDataToProvider(
        height: height,
        weight: weight,
        age: age,
        tdee: tdee,
      ...
      setState(() {
        ...
        if (height == null || height <= 0) {
          _recommendation = 'Lütfen geçerli bir boy değeri girin.';
        } else if (weight == null || weight <= 0) {
          _recommendation = 'Lütfen geçerli bir kilo değeri girin.';
        } else if (age == null || age <= 0) {
          _recommendation = 'Lütfen geçerli bir yaş değeri girin.';
        } else if (_gender.isEmpty) {
          _recommendation = 'Lütfen cinsiyet seçin.';
        } else if (_activityLevel.isEmpty) {
          _recommendation = 'Lütfen aktivite seviyesi seçin.';
        } else if (_purpose.isEmpty) {
          _recommendation = 'Lütfen hedef seçin.';
        } else {
          _recommendation = 'Lütfen tüm alanları doldurun.';
        }
      });
    }
  }
  ...
  void _loadUserDataFromProvider() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Only populate fields if user has actually entered meaningful data
      // (not default/empty values)
      ...
      // Always update dropdown values if they exist in Firebase (not empty)
  ...
  /// Hesaplanan verileri UserProvider'a kaydeder
  void _saveCalculatedDataToProvider({
    ...
    // Note: Macro values are calculated but not stored in User model
    // They are displayed in the analytics results
  }
