...
class OptimizationProvider extends ChangeNotifier {
  ...
  void updateItemQuantity(String itemId, int quantity) {
    ...
    if (index != -1 && quantity > 0) {
      _shoppingItems[index] =
          _shoppingItems[index].copyWith(quantity: quantity);
      _saveShoppingList();
      notifyListeners();
    }
  }
  ...
  void checkAllItems() {
    _shoppingItems =
        _shoppingItems.map((item) => item.copyWith(isChecked: true)).toList();
    notifyListeners();
  }
  ...
  void uncheckAllItems() {
    _shoppingItems =
        _shoppingItems.map((item) => item.copyWith(isChecked: false)).toList();
    notifyListeners();
  }
  ...
  List<ShoppingItem> filterByCalorieRange(
